...
```{plot deathrates, r, fig.width = 7}
deathrates_ken<-peeps::load_deathrates(country="Kenya")

# compare source data with interpolated points
deathrates_ken$interpolated<-ifelse(deathrates_ken$year %% 5 == 0, "base",
                                       "interpolated")

ken_plot<-ggplot2::ggplot(data=deathrates_ken, aes(year,value,
                     color=as.factor(deathrates_keninterpolated), 
                     fill=as.factor(deathrates_keninterpolated))) +
              geom_point()+
              labs(title = paste0(deathrates_kencountry, " death rates, ages: ",
                                  deathrates_kenage_from,
                                  "-",deathrates_kenage_to, " years"),
                   fill = "Interpolated",
                   color = "Interpolated")+
              theme_bw()
